

#include "../../set2/Challenge10/aes.c"
#include "../../set2/Challenge12/base64.c"

unsigned char text[3841] = "";

unsigned char* edit(unsigned char* originalCiphertext, int originalCiphertextLength,int index, unsigned char character){
    if(index >= originalCiphertextLength){
        exit(EXIT_FAILURE);
    }
    unsigned char* deciphered = aes_ctr_transform(originalCiphertext,originalCiphertextLength,"YELLOW SUBMARINE",0);
    deciphered[index] = character;
    unsigned char* ciphertext = aes_ctr_transform(deciphered,originalCiphertextLength,"YELLOW SUBMARINE",0);
    free(deciphered);
    return ciphertext;
}

int main(){

    int aesEcbCiphertextLength = getRawByteLengthFromBase64String(text);
    unsigned char* aesEcbCiphertext = convertBase64ToRawBytes(text);
    unsigned char* aesEcbPlaintext = aes_ecb_decrypt(aesEcbCiphertext,aesEcbCiphertextLength,"YELLOW SUBMARINE");

    int originalCiphertextLength = aesEcbCiphertextLength;
    unsigned char* originalCiphertext = aes_ctr_transform(aesEcbPlaintext,aesEcbCiphertextLength,"YELLOW SUBMARINE",0);

    unsigned char* keystream = calloc(originalCiphertextLength,1);
    for(int i=0;i<originalCiphertextLength;i++){
        unsigned char* modifiedCiphertext = edit(originalCiphertext,originalCiphertextLength,i,'A');
        keystream[i] = modifiedCiphertext[i] ^ 'A';
        free(modifiedCiphertext);
    }
    unsigned char* result = xorRawStrings(keystream, originalCiphertext,originalCiphertextLength);
    for(int i=0;i<originalCiphertextLength;i++){
        printf("%c",result[i]);
    }
    printf("\n");
    return 0;
}